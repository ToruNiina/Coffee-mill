#ifndef COFFEE_MILL_NINFO_CONTACT
#define COFFEE_MILL_NINFO_CONTACT
...
namespace coffeemill
{
    ...
    public:
        ...
        void set_imps(std::vector<int> imps)
        {
            ...
            if(imps.size() > 2) 
                std::cout << "Warning: set_imps has more than 2 argument in ContactLine"
                          << std::endl;
            imp1 = imps.at(1);
            imp2 = imps.at(2);
            return;
        ...
        void set_impuns(std::vector<int> impuns)
        {
            ...
            if(impuns.size() > 2) 
                std::cout << "Warning: set_impuns has more than 2 argument in ContactLine"
                          << std::endl;
            imp1un = impuns.at(1);
            imp2un = impuns.at(2);
            return;
    ...
    LineType ContactLine::readline(std::ifstream& file)
    {
        ...
        if(head != "contact")
        {
            ...
            std::cout << "this line is not contact line." << std::endl;
    ...
    void ContactLine::write_stream(std::ostream& os)
    {
        os << "contact";
    ...
    void ContactLine::read_stream(std::istream& is)
    {
    ...
    {
    public:
        ContactBlock(): BlockBase(N_CONTACT) {}
        ~ContactBlock(){}
    ...
    void ContactBlock::write_block(std::ofstream& os)
    {
        if(lines.empty())
        {
    ...
    void ContactBlock::read_block(std::ifstream& is)
    {
        // it is expected that
        ...
        while(!is.eof())
        {
            ...
            case EMPTY:
                break;
            case WRONG:
                throw std::invalid_argument("File Reading Error. contact block in ninfo file has invalid line.");
            case BLOCK_START:
                throw std::invalid_argument("File Reading Error. contact block in ninfo file is not closed by <<<<.");
            default:
                throw std::invalid_argument("File Reading Error. Unknown LineType.");
            }
        }
        throw std::invalid_argument("File Reading Error. contact block in ninfo file did not end correctly.");
    }
}
#endif //COFFEE_MILL_NINFO_CONTACT
