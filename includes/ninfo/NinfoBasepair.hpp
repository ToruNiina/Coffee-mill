#ifndef COFFEE_MILL_NINFO_BASEPAIR
#define COFFEE_MILL_NINFO_BASEPAIR
...
namespace coffeemill
{
    ...
    public:
        ...
        void set_imps(std::vector<int> imps)
        {
            ...
            if(imps.size() > 2) 
                std::cout << "Warning: set_imps has more than 2 argument in BasepairLine"
                          << std::endl;
            imp1 = imps.at(1);
            imp2 = imps.at(2);
            return;
        ...
        void set_impuns(std::vector<int> impuns)
        {
            ...
            if(impuns.size() > 2) 
                std::cout << "Warning: set_impuns has more than 2 argument in BasepairLine"
                          << std::endl;
            imp1un = impuns.at(1);
            imp2un = impuns.at(2);
            return;
    ...
    LineType BasepairLine::readline(std::ifstream& file)
    {
        std::string line;
        ...
        if(head != "basepair")
        {
            ...
            std::cout << "this line is not basepair line." << std::endl;
    ...
    void BasepairLine::write_stream(std::ostream& os)
    {
        os << "basepair";
        ...
        os << " " << std::setw(4) << this->bptype << " " << this->nhb_bp;
        os << std::endl;
        return;
    }

    void BasepairLine::read_stream(std::istream& is)
    {
        ...
        is >> this->bptype >> this->nhb_bp;
        return;
    }


    class BasepairBlock : public BlockBase
    {
    public:
    ...
    void BasepairBlock::write_block(std::ofstream& os)
    {
        if(lines.empty())
        {
    ...
    void BasepairBlock::read_block(std::ifstream& is)
    {
        // it is expected that
        ...
        while(!is.eof())
        {
            LineSptr line(new BasepairLine);
            ...
            case EMPTY:
                break;
            case WRONG:
                throw std::invalid_argument("File Reading Error. basepair block in ninfo file has invalid line.");
            case BLOCK_START:
                throw std::invalid_argument("File Reading Error. basepair block in ninfo file is not closed by <<<<.");
            default:
                throw std::invalid_argument("File Reading Error. Unknown LineType.");
            }
        }
        throw std::invalid_argument("File Reading Error. basepair block in ninfo file did not end correctly.");
    }
}
#endif //COFFEE_MILL_NINFO_BASEPAIR
